  ...
  return (
    ...
    <div className="flex flex-wrap mx-4">
      ...
      posts.map((item, index) => (
        ...
        <div key={index} href={`/products/${item.id}`} className="w-full sm:w-1/2 md:w-1/2 xl:w-1/4 p-2">
...
export async function getStaticPaths() {
  ...
  const paths = apiData.data.listProducts.items.map(product => ({ params: { id: product.id }}))
  return {
    paths,
    fallback: true
  }
...
export async function getStaticProps ({ params }) {
  ...
  return {
    props: {
      product: apiData.data.getProduct
    }
  }
}